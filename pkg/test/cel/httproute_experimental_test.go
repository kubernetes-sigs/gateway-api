...
func TestHTTPRouteHeaderValueExperimental(t *testing.T) {
	tests := []struct {
		name        string
		wantErrors  []string
		headerValue gatewayv1.HTTPHeaderValue
	}{
		{
			name:        "valid with multiple values",
			wantErrors:  []string{},
			headerValue: "text/plain; charset=UTF-8, application/xml",
		},
		{
			name:        "valid with special characters",
			wantErrors:  []string{},
			headerValue: "attachment; filename=\"example file.txt\"",
		},
		{
			name:        "invalid because of non-print characters (newline)",
			wantErrors:  []string{"must only contain printable ASCII characters, optionally separated by single tabs or spaces"},
			headerValue: "this\r\nis\ninvalid\r",
		},
	}
