...
.mkdocs.timestamp: images $(SOURCES)
	$(DOCKER) run \
		...
		--rm \
		$(MKDOCS_IMAGE) \
		/bin/bash -c "cd /d && $(MKDOCS) build && sed -i -e '/REPLACE_WITH_GENERATED_CONTENT/{r docs-src/_spec.html' -e 'd}' docs/spec/index.html; find /d/docs -exec chown $(UID):$(GID) {} \;"
...
verify: images
	$(DOCKER) run \
		...
		--rm \
		$(MKDOCS_IMAGE) \
		/bin/bash -c "cd /d && $(MKDOCS) build --site-dir=/tmp/d && sed -i -e '/REPLACE_WITH_GENERATED_CONTENT/{r docs-src/_spec.html' -e 'd}' /tmp/d/spec/index.html && diff -X /d/docs-src/.mkdocs-exclude -qr /d/docs /tmp/d "
