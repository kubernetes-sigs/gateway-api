...
func TestHTTPPathMatch(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			path: &gatewayv1.HTTPPathMatch{
				Type:  new(gatewayv1.PathMatchType("FooBar")),
				Value: new("/path"),
			},
		},
		{
			...
			path: &gatewayv1.HTTPPathMatch{
				Type:  new(gatewayv1.PathMatchType("RegularExpression")),
				Value: new("/foo/./bar"),
			},
		},
		{
		...
		},
		{
			...
			path: &gatewayv1.HTTPPathMatch{
				Type:  new(gatewayv1.PathMatchType("Exact")),
				Value: new("/abc/123'/a-b-c/d@gmail/%0A"),
			},
		},
		{
...
func TestHTTPRouteFilter(t *testing.T) {
	tests := []struct {
		...
		{
			name: "invalid HTTPCORSFilter type filter with non-matching field",
			routeFilter: gatewayv1.HTTPRouteFilter{
				Type:          gatewayv1.HTTPRouteFilterCORS,
				RequestMirror: &gatewayv1.HTTPRequestMirrorFilter{},
			},
			wantErrors: []string{"filter.cors must be specified for CORS filter.type", "filter.requestMirror must be nil if the filter.type is not RequestMirror"},
		},
		{
			name: "invalid HTTPCORSFilter type filter with empty value field",
			routeFilter: gatewayv1.HTTPRouteFilter{
				Type: gatewayv1.HTTPRouteFilterCORS,
			},
			wantErrors: []string{"filter.cors must be specified for CORS filter.type"},
		},
		{
			name: "valid HTTPCORSFilter test",
			routeFilter: gatewayv1.HTTPRouteFilter{
				Type: gatewayv1.HTTPRouteFilterCORS,
...
func TestHTTPRouteRule(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			rules: []gatewayv1.HTTPRouteRule{{
				Matches: []gatewayv1.HTTPRouteMatch{
					{
						Path: &gatewayv1.HTTPPathMatch{
							Type:  new(gatewayv1.PathMatchRegularExpression), // Incorrect Path match Type for URLRewrite filter with ReplacePrefixMatch.
							Value: new("/foo"),
						},
					},
				},
		...
		},
		{
			...
			rules: []gatewayv1.HTTPRouteRule{{
				Matches: []gatewayv1.HTTPRouteMatch{
					{
						Path: &gatewayv1.HTTPPathMatch{
							Type:  new(gatewayv1.PathMatchRegularExpression), // Incorrect Path match Type for RequestRedirect filter with ReplacePrefixMatch.
							Value: new("/foo"),
						},
					},
				},
		...
		},
		{
			...
			rules: []gatewayv1.HTTPRouteRule{
				{
					Name: new(gatewayv1.SectionName("name1")),
				},
				{
					Name: new(gatewayv1.SectionName("name1")),
				},
		...
		},
		{
			...
			rules: []gatewayv1.HTTPRouteRule{
				...
				{
					Name: new(gatewayv1.SectionName("not-name1")),
				},
				{
					Name: new(gatewayv1.SectionName("name1")),
				},
		...
		},
		{
			...
			rules: []gatewayv1.HTTPRouteRule{
				...
				{Name: nil},
				{Name: new(gatewayv1.SectionName("name1"))},
				{Name: new(gatewayv1.SectionName("name2"))},
			},
		},
	}
...
func TestHTTPRouteCORS(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErrors: []string{"Invalid value: \"http://*.bar.*.dev\": spec.rules[0].filters[0].cors.allowOrigins[0] in body should match"},
		},
		{
