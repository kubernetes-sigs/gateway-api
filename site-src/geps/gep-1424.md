...
* SHOULD only be allowed to direct traffic to `backendRefs` within the same namespace when used for configuration of mesh traffic, setting a `ResolvedRefs` condition with `status: "False"` and reason `RefNotPermitted` on the `HTTPRoute` if this is attempted.
