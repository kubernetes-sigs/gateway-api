...
```go
  ...
  // as one of the route kinds. It is invalid to reference the same parent
  // resource more than once.

  // It is possible to separately reference 2 distinct objects that may be
  // collapsed by an implementation. For example, some implementations may
  // choose to merge compatible listeners together. If that is the case, the
  // list of routes attached to those resources should also be merged.
  //
  // +optional
  // +kubebuilder:validation:MaxItems=16
...
rules to support the SectionName concept:
