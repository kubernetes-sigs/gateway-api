...
* Augment the Gateway API specification with the necessary models, resources, and modifications so that the xRoute primitives are useable by service mesh implementations
* Ensure common policies applicable to both north/south and east/west are respected
...
* Flesh out egress gateway story
  * How traffic gets to the gateway may start out as vendor specific, but perhaps Gateway API should standardize on exposing Routes for the egress gateway?

## Open Questions

* Should creating a set of policy attachments for table stakes functionality (e.g. authorization policy) be a goal?
  * If so, what’s the process/logic for deciding which features have community-supported policy attachments and which ones are left up to the vendor
...
<div float="left">
  <figure style="display:inline-block;float:left;text-align: center; width: 45%;">
