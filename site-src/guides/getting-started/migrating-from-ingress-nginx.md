...
Many annotation-based features in Ingress-NGINX have corresponding fields in Gateway API. For example, traffic splitting, header manipulation, and TLS configuration are all native to the API. For a detailed mapping, we recommend exploring the [Gateway API HTTPRoute documentation](../../api-types/httproute/) and the documentation of your chosen [implementation](https://gateway-api.sigs.k8s.io/implementations/).
