...
# Feature definitions for the wizard: id, label, and optional description (tooltip).
...
FEATURE_DEFINITIONS = {
    "httpGateway": [
        ...
        {"id": "GatewayAddressEmpty", "label": "Gateway Address Empty", "description": "Gateway status supports empty address list."},
    ...
    "httpRoute": [
        ...
        {"id": "HTTPRoutePathRedirect", "label": "HTTPRoute Path Redirect", "description": "Issue HTTP redirects (e.g. 302) with a modified path."},
        ...
        {"id": "HTTPRouteSchemeRedirect", "label": "HTTPRoute Scheme Redirect", "description": "Redirect HTTP to HTTPS (or other scheme)."},
        {"id": "HTTPRouteBackendProtocolH2C", "label": "HTTPRoute Backend Protocol H2C", "description": "Use HTTP/2 (cleartext) to backends."},
        {"id": "HTTPRouteBackendProtocolWebSocket", "label": "HTTPRoute Backend Protocol Web Socket", "description": "WebSocket support to backends."},
        {"id": "HTTPRouteBackendRequestHeaderModification", "label": "HTTPRoute Backend Request Header Modification", "description": "Modify request headers sent to backends."},
        {"id": "HTTPRouteBackendTimeout", "label": "HTTPRoute Backend Timeout", "description": "Request timeout per backend."},
        {"id": "HTTPRouteCORS", "label": "HTTPRoute CORS", "description": "Cross-Origin Resource Sharing (CORS) policy."},
        {"id": "HTTPRouteDestinationPortMatching", "label": "HTTPRoute Destination Port Matching", "description": "Match requests by destination port."},
        {"id": "HTTPRouteMethodMatching", "label": "HTTPRoute Method Matching", "description": "Match requests by HTTP method (GET, POST, etc.)."},
        {"id": "HTTPRouteNamedRouteRule", "label": "HTTPRoute Named Route Rule", "description": "Reference route rules by name for ordering."},
        {"id": "HTTPRouteParentRefPort", "label": "HTTPRoute Parent Ref Port", "description": "Attach to a specific Gateway listener port."},
        {"id": "HTTPRoutePathRewrite", "label": "HTTPRoute Path Rewrite", "description": "Rewrite the path of requests sent to backends."},
        {"id": "HTTPRoutePortRedirect", "label": "HTTPRoute Port Redirect", "description": "Redirect to a different port."},
        {"id": "HTTPRouteQueryParamMatching", "label": "HTTPRoute Query Param Matching", "description": "Match requests by query parameters."},
        {"id": "HTTPRouteRequestMultipleMirrors", "label": "HTTPRoute Request Multiple Mirrors", "description": "Mirror traffic to multiple backends."},
        {"id": "HTTPRouteRequestTimeout", "label": "HTTPRoute Request Timeout", "description": "Timeout for the entire request."},
    ],
    "httpBackendTls": [
        {"id": "BackendTLSPolicy", "label": "Backend TLS Policy", "description": "Configure TLS when connecting to backends."},
        {"id": "BackendTLSPolicySANValidation", "label": "Backend TLS Policy SAN Validation", "description": "Validate backend certificate SANs."},
    ],
    "grpc": [
        {"id": "GatewayAddressEmpty", "label": "Gateway Address Empty", "description": "Gateway status supports empty address list."},
        {"id": "GatewayHTTPListenerIsolation", "label": "Gateway HTTP Listener Isolation", "description": "GRPCRoutes can target a single listener; others are isolated."},
        {"id": "GatewayInfrastructurePropagation", "label": "Gateway Infrastructure Propagation", "description": "Gateway status propagates to GRPCRoutes."},
        {"id": "GatewayPort8080", "label": "Gateway Port 8080", "description": "Gateway listens on port 8080."},
        {"id": "GatewayStaticAddresses", "label": "Gateway Static Addresses", "description": "Gateway supports static address assignment."},
    ],
    "tls": [
        {"id": "GatewayAddressEmpty", "label": "Gateway Address Empty", "description": "Gateway status supports empty address list."},
        {"id": "GatewayHTTPListenerIsolation", "label": "Gateway HTTP Listener Isolation", "description": "TLSRoutes can target a single listener; others are isolated."},
