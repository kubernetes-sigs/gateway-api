...
func init() {
	ConformanceTests = append(ConformanceTests, TCPRouteValidInvalidListeners)
}

var TCPRouteValidInvalidListeners = confsuite.ConformanceTest{
	ShortName:   "TCPRouteValidInvalidListeners",
	Description: "A TCPRoute should attach to the TCP listener on a Gateway that also has a UDP listener on a different port. The UDP listener must remain unattached and not routable.",
	Manifests:   []string{"tests/tcproute-valid-invalid-listeners.yaml"},
	...
	Test: func(t *testing.T, suite *confsuite.ConformanceTestSuite) {
		...
		gwNN := types.NamespacedName{Name: "tcp-udp-listeners-gateway", Namespace: ns}
		routeNN := types.NamespacedName{Name: "tcp-route-on-mixed-listener-gateway", Namespace: ns}
		...
		t.Run("TCPRoute attaches to the TCP listener on a Gateway with both TCP and UDP listeners", func(t *testing.T) {
			kubernetes.TCPRouteMustHaveParents(t, suite.Client, suite.TimeoutConfig, routeNN, expectedParents, false)
		})

		t.Run("TCP listener has 1 attached route and UDP listener has 0 attached routes", func(t *testing.T) {
		...
		t.Run("TCP traffic is forwarded through the TCP listener to the backend", func(t *testing.T) {
