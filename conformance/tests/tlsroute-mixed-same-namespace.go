...
func init() {
	ConformanceTests = append(ConformanceTests, TLSRouteMixedSameNamespace)
}

var TLSRouteMixedSameNamespace = suite.ConformanceTest{
	ShortName:   "TLSRouteMixedSameNamespace",
	...
	Provisional: true,
	Manifests:   []string{"tests/tlsroute-mixed-same-namespace.yaml"},
	Test: func(t *testing.T, suite *suite.ConformanceTestSuite) {
		...
		gwAddrPassthrough, hostnamesPassthrough := kubernetes.GatewayAndTLSRoutesMustBeAccepted(t, suite.Client, suite.TimeoutConfig, suite.ControllerName,
		...
		t.Run("Simple MQTT TLS request matching TLSRoute should reach mqtt-backend", func(t *testing.T) {
			...
			t.Logf("Establishing MQTT connection to host %s via %s", serverStrTerminate, gwAddrPassthrough)
			...
			opts.AddBroker(fmt.Sprintf("tls://%s", gwAddrPassthrough))
		...
		t.Run("Simple TLS request matching TLSRoute Passthrough should reach infra-backend", func(t *testing.T) {
			t.Parallel()
			tlsutils.MakeTLSRequestAndExpectEventuallyConsistentResponse(t, suite.RoundTripper, suite.TimeoutConfig, gwAddrPassthrough, serverCertPem, nil, nil, serverStrPassthrough,
