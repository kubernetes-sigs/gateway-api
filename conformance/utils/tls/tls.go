...
func MakeTLSRequestAndExpectFailure(t *testing.T, r roundtripper.RoundTripper, timeoutConfig config.TimeoutConfig, gwAddr string, cPem, keyPem []byte, server string, expected http.ExpectedResponse) {
	...
	WaitForConsistentTLSFailureResponse(t, r, req, timeoutConfig.RequiredConsecutiveSuccesses, timeoutConfig.MaxTimeToConsistency, cPem, keyPem, server)
}
...
// that after maxTimeToConsistency the response doesn't returns HTTP Status OK (200)
func WaitForConsistentTLSFailureResponse(t *testing.T, r roundtripper.RoundTripper, req roundtripper.Request, threshold int, maxTimeToConsistency time.Duration, cPem, keyPem []byte, server string) {
	req.KeyPem = keyPem
	req.CertPem = cPem
	req.Server = server
	http.WaitForConsistentFailureResponse(t, r, req, threshold, maxTimeToConsistency)
}
