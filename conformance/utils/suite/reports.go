...
type profileReportsMap map[ConformanceProfileName]confv1b1.ProfileReport
...
func (p profileReportsMap) addTestResults(conformanceProfile ConformanceProfile, result testResult) {
	...
	if _, ok := p[conformanceProfile.Name]; !ok {
		p[conformanceProfile.Name] = confv1b1.ProfileReport{
	...
	case testSucceeded:
		if testIsExtended {
			if report.Extended == nil {
				report.Extended = &confv1b1.ExtendedStatus{}
			}
...
func (p profileReportsMap) list() (profileReports []confv1b1.ProfileReport) {
...
func (p profileReportsMap) compileResults(supportedFeaturesMap map[ConformanceProfileName]sets.Set[SupportedFeature], unsupportedFeaturesMap map[ConformanceProfileName]sets.Set[SupportedFeature]) {
	for key, report := range p {
		...
		case report.Core.Failed > 0:
			report.Core.Result = confv1b1.Failure
		case report.Core.Skipped > 0:
			report.Core.Result = confv1b1.Partial
		default:
			report.Core.Result = confv1b1.Success
		}
		...
		if report.Extended != nil {
			...
			case report.Extended.Failed > 0:
				report.Extended.Result = confv1b1.Failure
			case report.Extended.Skipped > 0:
				report.Extended.Result = confv1b1.Partial
			default:
				report.Extended.Result = confv1b1.Success
			}
		}
...
func buildSummary(report confv1b1.ProfileReport) (reportSummary string) {
...
func buildReportSummary(status confv1b1.Status) string {
	var message string
	switch status.Result {
	case confv1b1.Success:
		message = "succeeded"
	case confv1b1.Partial:
		message = fmt.Sprintf("partially succeeded with %d test skips", status.Statistics.Skipped)
	case confv1b1.Failure:
