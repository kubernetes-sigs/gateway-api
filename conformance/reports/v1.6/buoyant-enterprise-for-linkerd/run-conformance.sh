...
# Set these as needed.
LINKERD_VERSION=${LINKERD_VERSION:-enterprise-2.20.2}
...
if [ "$installed_version" != "$LINKERD_VERSION" ]; then
  echo "Install Linkerd CLI version $LINKERD_VERSION" >&2
  exit 1
else
  echo "Linkerd CLI version $LINKERD_VERSION is already installed"
fi
...
# linkerd install --set proxy.nativeSidecar=false | kubectl apply -f -
