...
This GEP initially proposes definitions for session persistence and session affinity, followed by the definition of
an API spec for configuring session persistence.
...
- Mandate a default session persistence functionality for implementations
...
including the gateway, backend, a sidecar in a backend, or any other infrastructure component.
...
In a more complex architecture, a global load balancer may initiate a session in order to maintain persistent
connections to a regional load balancer. The regional cluster load balancer can then use the cookie to maintain
persistent connections to a specific backend.

Here an example implementation of a global load balancer initiating a session through cookies:
```mermaid
sequenceDiagram
    actor C as Client
...
implementations must consider how to manage sessions initiated by other components. This GEP leaves the decision of
managing existing sessions with each specific implementation. In the case of cookie-based session persistence, an
implementation has the freedom to either rewrite the cookie or insert an additional cookie, or to do nothing (resulting
in the lack of a persistent session).
