...
For both header-based and cookie-based sessions, the initiation of session persistence is possible from various sources,
including the gateway, backend, a sidecar in a backend, or any other infrastructure component. The client application
also plays in a role in session initiation. If a client doesn't want a request to belong an existing session, it can
remove the session cookie or identifier to signal the gateway or backend to recreate the session.

Let's focus on the two primary sources: gateways and the backend. The following rules apply based on who initiates the session:
...
When dealing with backend-initiated sessions, the process becomes somewhat more complex. The gateway must modify the
cookie's value to establish persistent sessions. However, in the most straightforward scenario, if the backend is not
configured for session persistence, it should refrain from modifying the cookie. In this situation, the gateway should
remain passive and simply forward the `set-cookie` header as it is.

Here's an example of where the backend initiating a session, but the gateway does not modify the cookie:
```mermaid
sequenceDiagram
    actor C as Client
    participant G as Gateway
    ...
    Note right of C: [cookie] indicates a request<br> with a cookie
    C->>+G: Request Web Page<br>[cookie]
    Note right of G: Gateway is not configured<br>for session persistence
    G->>+B: Request<br>[cookie]
    B-->>-G: Response
    G-->>-C: Response
```

Should an application developer wish for the gateway to establish a persistent session, even when a cookie with the same
name is generated by the backend server, the gateway will need to modify the set-cookie header provided by the backend 
server. The primary motivation behind gateways updating the cookie is to rewrite the cookie value with a server
identifier in order to a maintain persistent session to a backend. There are several approaches to updating a cookie's
value. The gateway could either overwrite the cookie's value entirely or prefix the cookie's name with a unique
identifier. Refer to the [Cookie Rewriting](#cookie-rewriting) section of the API for implementation guidance.

Here's an example of where the backend initiating a session and the gateway modifies the cookie:
```mermaid
sequenceDiagram
    actor C as Client
    participant G as Gateway
...
These Java applications introduced the “jsessionid” cookie as well as more advanced features such as session migration,
replication, and on demand session activation. It’s important for Gateway API to examine cookie use cases and history
from Java APIs to ensure the API is designed appropriately.
...
particular application of session affinity doesn't impact the design of the GEP. However, we recommend that users enable
...
type SessionPersistencePolicySpec struct {
    ...
    // references.
	//
	// Services as target reference are core support level.
	// HTTPRoutes and GRPCRoute as target reference are extended
	// support level.
    ...
    // Duration defines the duration of the persistent session.
    //
    // Support: Core
    //
    // +optional
...
infrastructure types, and some implementations that already provide session persistence may not be able to adhere to
low-level API.
...
The metaresource is named `SessionPersistencePolicy` and is only responsible for configuring session persistence for services.
It is defined as a [Direct Policy Attachment](https://gateway-api.sigs.k8s.io/v1alpha2/references/policy-attachment/#direct-policy-attachment)
...
it to a route will be considered extended. This distinction arises because most existing implementations primarily
support attaching to a service, while attaching to a route is less common and involves greater complexities (see [API Attachment Point: Route vs. Service](#api-attachment-point-route-vs-service)
for more details).
...
attached to a route, it can define session persistence for multiple services, whereas attaching it to a service
defines it for a single service.

Attaching to a services represents a core API support level whereas attaching to a route (e.g. HTTPRoutes and GRPCRoutes)
represents an extended API support level. Implementations must support services for conformance, while routes are
considered optional.
...
implementations. However, certain cookie attributes require special consideration, which we will discuss in detail.
...
and setting `SameSite` to `Strict`.
...
Cookie rewriting happens when the backend generates the cookie with the same name before the gateway has the opportunity
to create it, and then the gateway modifies the cookie's value and/or attributes. During the implementation of this
specification, it is up to the individual implementations to determine how to handle such cases. An implementation has
the freedom to either rewrite the cookie or choose not to modify it, resulting in the absence of a persistent session.
