...
We are anticipating the possibility of future enhancements in supporting the selection of the [`matches`](https://gateway-api.sigs.k8s.io/references/spec/#gateway.networking.k8s.io/v1beta1.HTTPRouteMatch)
segment within xRoutes, contingent upon updates to [GEP-713](/geps/gep-713) for incorporating section targeting (PR [#2283](https://github.com/kubernetes-sigs/gateway-api/pull/2283)).
Enabling the attachment to the `matches` route sections will enable a more granular association of session persistence
to route paths. Without the capability to target a specific `matches` section within a route, users might be required to
...
predicable way.
- Update the GEP for targeting a route `matches` section when [#2283](https://github.com/kubernetes-sigs/gateway-api/pull/2283)
