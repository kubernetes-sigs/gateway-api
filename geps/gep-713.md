...
(which conceiably could be the whole cluster):
...
it is a Policy object.

The label is `gateway.networking.k8s.io/policy: true`.
...
That Condition must have a `type` of `gateway.networking.k8s.io/PolicyAffected`,
...
`gateway.networking.k8s.io/PolicyAffected: true` added instead.
...
This solution should also be considered experimental and needs to be accepted by
the community. (This comment will be removed before merge).
...
This solution should also be considered experimental and needs to be accepted
by the community. (This comment will be removed before merge).
...
That Condition must have a `type` of `gateway.networking.k8s.io/PolicyAffected`, and have the optional `observedGeneration` field kept up to date when the `spec` of the Policy-attached object changes.

For objects that do _not_ have a `status.Conditions` field available (`Secret` is a good example), that object MUST instead have an annotation of `gateway.networking.k8s.io/PolicyAffected: true` added instead.
