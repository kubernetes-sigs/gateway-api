...
in multiple Routes, in multiple Gateways. However, the status of the `BackendTLSPolicy`
will be different only at the Gateway level, so Gateway is the relevant Ancestor
for the status.
...
type PolicyAncestorStatus struct {
	...
	AncestorRef ParentReference `json:"parentRef"`
