...
Additionally, if an implemntation sees an update that changes the `status`` of this
Condition from `false` to `true` _that it did not create_, it must not attempt
to change the status back to `false`.

This is necessary because it's possible that multiple controllers may be trying
