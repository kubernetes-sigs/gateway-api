...
< The Diagram with Policy Admin added goes here >

From the Policy Admin’s point of view, they need to know across their whole remit (which conceiably could be the whole cluster):
...
An important note here is that a key piece of information for Policy Admins and Cluster Operators is “How many things does this Policy affect?”. In the parable, this would have enabled Julian to know that deleting the Namespace Policy would affect many other people than just Jane.

### Problems solutions need to solve
...
How does Jane, or any Application Devloper who owns one or more Route objects know that their object is affected by Policy, which Policy is affecting it, and what the content of the Policy is?

The best outcome is that Jane needs to look only at a specific route to know what Policy settings are being applied to that Route, and where they come from.
However, some of the other problems below make it very difficult to achieve this.

##### Policy Admin Discoverability

How does the Policy Admin know what Policy is applied where, and what the content of that Policy is?
...
For any given Policy type, whether Direct Attached or Inherited, implementations will need to be able to _calculate_ the resultant set of Policy to be able to apply that Policy to the correct parts of their data plane configuration.
However, _displaying_ that resultant set of Policy in a way that is straightforward for the various personas to consume is much harder.
...
- Updating one Policy object could cause many affected objects to need to be updated themselves. This sort of fan-out problem can be very bad for apiserver load, particularly if Policy chnges rapidly, there are a lot of objects, or both.
...
The problem here is that we need to have a _common_ pattern for including the details of an _arbitrarily defined_ object, that needs to be included in the base API.
...
##### Fan-out status update problems
...
A standard Condition (`gateway.networking.k8s.io/PolicyAffected` or similar), which would allow things we can’t change the spec of to have some indication.

Everyone currently seems to feel that this is a good idea and we should go ahead.

#### `kubectl` plugin or command-line tool
@robscott suggested that this would be:
- Relatively achievable
- A good reference for implementations (particularly if we provide a library as part of it)
- A good way to try out patterns and see what’s useful

#### Standard status stanza
A status stanza on things we can change that lists the Policy objects affecting an object.
...
An object that holds the resultant set of Policy for an object, could use convention instead of direct reference to be able to find this object (like Services and Endpoints).

#### GatewayClass status Extension Types listing
We mandate that every implementation must list all extra types in its GatewayClass status (like Policy, and other extension types, like paramsRef targets, filters, and so on). We should probably do this anyway, as it’s a great way to keep implementations honest about what extra stuff they are providing.
...
### Immediate Changes
Describe a standard Condition in this document and PR the details of the Condition into the Go types.
Design a GatewayClass extensions list. (Should possibly be a small, separate GEP since it’s applicable to more things than just Policy).
