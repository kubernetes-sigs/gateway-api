...
type HTTPRouteTimeouts struct {
	// Request specifies an end-to-end timeout for client HTTP requests,
	// disabled by default.
	//
	// For example, the following rule will wait no longer than 10 secs for the
	...
	// BackendRequest specifies a timeout for an individual request to a backend service.
...
1. `timeouts.backendRequest` is a timeout for a single backend request as measured by the gateway, not the client.
    This field is optional `Extended` support. Typically used in conjuction with retry configuration, if supported
    by an implementation. Note that retry configuration will be the subject of a separate GEP (GEP-1731).
