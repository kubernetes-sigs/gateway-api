...
Each implementation's regex flavor MUST be a superset of [IEEE POSIX ERE](https://pubs.opengroup.org/onlinepubs/9699919799/basedefs/V1_chap09.html).
...
The only exception is that we do not require implementations be leftmost-longest or leftmost-first.
That is, we do not define how implementations should deal with ambiguous matches.
Given the pattern `a|aa|aaa`, there could be one, two or three matches on the string `aaa`.
