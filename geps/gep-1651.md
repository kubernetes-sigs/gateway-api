...
Implementations MAY leave the `spec.infrastructure.routability` property unset and instead
signal the default routability using the Gateway's `status.addresses`. See 'Status Addresses`
for more details.
...
`Accepted`, `Ready`, `Programmed` to `False` with `Reason` set to `Invalid`.
...
type GatewayInfrastructure struct {
  // Routability allows the Gateway to specify the accessibility of it's addresseses
...
If a Gateway is unable to provide an address for the desired routability it MUST set the condition `Accepted` to `False` with `Reason` set to `Invalid`
