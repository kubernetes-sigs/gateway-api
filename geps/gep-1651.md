...
Value | Semantic
-|-
`public`|The address is reachable over the public internet
...
by a slash `/` and a custom name ie. `com.example.com/my-reachability`.
...
### Mutability
Implementations MAY prevent end-users from updating the `reachability` value of a Gateway. If
...
reachability choices of a Gateway. If the desired Gateway reachability is incompatible with the
...
If a Gateway is unable provide an address for the desired reachability it MUST set the condition `Accepted` to `False` with `Reason` set to `Invalid`

#### Status.Addresses

If a Gateway supports the desired 'reachability' implementations MUST populate the `status.addresses` with
an address with the desired reachability semantic.
...
### Adding `reachabiliy` attribute to GatewayClass
...
reachability. Thus GatewayClass name is a viable option to control reachability. But it is not
flexible enough for implementations that can configure reachability when creating Gateways.
Otherwise implementations would be forced to create unnecessary GatewayClasses that may cause
redundant infrastructure to be provisioned.
...
`foo-cluster` are two GatewayClasses surfacing the values of a single attribute `reachability`.
