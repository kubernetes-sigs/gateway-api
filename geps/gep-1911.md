...
1. Open Question: TLS over UDP seems to be a thing via QUIC/HTTP3 [ref](https://www.smashingmagazine.com/2021/08/http3-core-concepts-part1/).
...
`TCP`  |`kubernetes.io/h2c` | `TLSRoute`  | Conditional [5]
...
`TCP`  |`kubernetes.io/ws`  | `TLSRoute`  | Conditional [5]
...
1. Only if there is a corresponding `TLSConnectionPolicy` - see [GEP-1897](geps/gep-1897)
2. GRPC works over h2c - so a GRPCRoute should be able to connect to an h2c backend
...
5. Only if the TLSRoute is attached to a Listener that has `tls.mode: Terminate` set.
