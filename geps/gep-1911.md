...
Not all implementations support automatic protocol selection. Even in some cases protocols are disabled without an explicit opt-in (eg. websockets). Thus application developers need the ability to specify the protocol(s) that their application supports.
...
We propose adding a new meta-resource [GEP-713](geps/gep-713.md) called `BackendProtocol`.
...
then it must set it's RouteConditionType `Accepted` to `False`, with a RouteConditionReason `UnsupportedProtocol`
...
that operate at a higher level (eg. HTTP/GRPC etc.)

### Kubernetes Service - Expanding AppProtocol field

Since Kubernetes 1.20 the core/v1.Service object has a stable `appProtocol` field. It's purpose is to allow end-users to
specify an application protocol (L7) for each service port.
...
Further discussion [here](https://github.com/kubernetes-sigs/gateway-api/discussions/1244#discussioncomment-3151197)

Since then a Kubernetes enhancement proposal was created [KEP-3726](https://github.com/kubernetes/enhancements/issues/3726)
to repurpose `appProtocol` to include a convention for protocols that are not IANA service names. This would involve
prefixing protocol names with `kubernetes.io/`. At the moment there exists two defined protocols `kubernetes.io/h2c` and `kubernetes.io/grpc`.

It is not possible to have different `appProtocol` entries for the same port. A related [KEP-1435](https://github.com/kubernetes/enhancements/tree/master/keps/sig-network/1435-mixed-protocol-lb) mentions different `protocol` entries for the same port. Finally it's unclear how one would disable certain protocol behaviours (ie. HTTP/1 Upgrades).
