...
Implementations MUST supported the following combinations below:
...
`TCP`  | `kubernetes.io/wss` | `HTTPRoute` | Conditional [2]

1. Only if the TLSRoute is attached to a Listener that has `tls.mode: Terminate` set.
2. Only if there is a corresponding `BackendTLSPolicy` - see [GEP-1897](geps/gep-1897)
