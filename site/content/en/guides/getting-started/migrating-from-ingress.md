...
{{% alert color="primary" title="Are you an Ingress-NGINX user?" %}}
If you are specifically using Ingress-NGINX, we recommend also checking out the [Ingress-NGINX Migration Guide](/guides/getting-started/migrating-from-ingress-nginx/) for more tailored advice and resources.
...
> Read more about the [design goals](/docs/introduction/#gateway-api-concepts)
> of Gateway API.
...
Gateway API
includes [four explicit personas](/docs/concepts/security/#roles-and-personas):
...
*   *External references.* A feature (field) of a Gateway API resource can
    ...
    *   [HTTPRouteFilter](/reference/api-spec/main/spec/#httproutefilter)
        can reference an external resource via the `extensionRef` field, thus
        configuring an implementation-specific filter.
    *   [BackendObjectReference](/reference/api-spec/main/spec/#backendobjectreference)
        supports resources other than Services.
    *   [SecretObjectReference](/reference/api-spec/main/spec/#secretobjectreference)
...
*   *Custom implementations*. For some features, it is left up to an
    ...
    (custom)  [conformance level](/docs/concepts/conformance/#2-support-levels). For
    example:
    *   The `RegularExpression` type of
        the [HTTPPathMatch](/reference/api-spec/main/spec/#httppathmatch).
...
a [Gateway](/reference/api-types/gateway/) resource. For example, if you want the data
plane to handle HTTP traffic on port 80, you need to define
a [listener](/reference/api-spec/main/spec/#listener) for
...
the [Gateway listener](/reference/api-spec/main/spec/#listener),
...
the [routing rules](/reference/api-spec/main/spec/#httprouterule)
of the [HTTPRoute](/reference/api-types/httproute/).
...
map to
the [hostnames](/reference/api-spec/main/spec/#hostname) of
...
> The `hostnames` of an HTTPRoute must match the `hostname` of the [Gateway listener](/reference/api-spec/main/spec/#listener).
> Otherwise, the listener will ignore the routing rules for the unmatched
> hostnames. See the [HTTPRoute documentation](/reference/api-spec/main/spec/#httproutespec).
...
*   Conflicts must be handled as
    prescribed in [API Design Guide: Conflicts](../api-design/#conflicts). For example, more specific
...
a [parentRef](/reference/api-spec/main/spec/#parentreference).
